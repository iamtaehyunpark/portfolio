...
public class HW6 {
	 ...
	 */
	 ...
	 /*
	  * This method will build tree by decreasing a character from expression and adding a Node to tree one by one
	  if input value is parenthesis, build tree for elements inside the parenthesis and combine them with the original tree
	  recursion will start with taking one character from the expression and making leftmost node.
	...
	public void buildTree(String expression) {
		...
		if (expression.length() == 0); // Base case, stop if all the characters are added into tree
			...
			if (expression.length() > 0) {
				...
				||expression.charAt(0) == '*' || expression.charAt(0) == '/') { // dealing with the operators
	 ...
	 */
	  ...
	  * This method will simply operates with two operand, left and right.
	  if the left or right value has a child node, operates them recursively until that node's element is a number and has no child
	 ...
	 */
	 ...
	 /*
	  * If root's child is operator, makes root's child as root and operates with its children recursively
	  */
	public double iterativeEval() {
		final Node temp = root;

		while(true){
			if(root.element instanceof Double) break;
	...
	public double helperIterEval(Node leftN, Node rightN, Node root){
		double left;
		double right;
		if(leftN.element instanceof Double) left = (double)leftN.element;
		else left = (char)leftN.element - '0';
		if(rightN.element instanceof Double) right = (double)rightN.element;
		else right = (char)rightN.element - '0';
